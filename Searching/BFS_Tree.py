# Uses additional memory as it is necessary for bfs to track the child node on a given level
# BFS is like a water flooding from the top

# BFS vs DFS : all taverslas for both is o(n) and o(logn) for searching
 #BFS is good for finding shortest path
 #searches closer nodes first
 # Requires more memory
 # Rule of thumb if we have addional infoabout the node is likely in upper level of tree

#DFS is beeter if goal is at the lower nodes 
# Less memory
#it answers does the path exists ?
# it is usyually slower if deep tree
#not good at finding the shortest path
# DFS recursively is much more efficient for space complecity O(height) in worst casr
# DFS implemented iteratively using stack has same space complexity as BFS  O(n) and time is just same for both
# for graph space complexity might vary alittile as there we need to store the visited edges as well.
